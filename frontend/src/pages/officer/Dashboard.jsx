...
import { Users, Briefcase, FileText, CheckCircle, Clock, TrendingUp, LogOut } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

export default function OfficerDashboard() {
  const { user, logout } = useAuth();
  const [stats, setStats] = useState({ totalStudents: 0, verifiedStudents: 0, activeJobs: 0, totalApplications: 0, placed: 0 });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="card bg-gradient-to-r from-blue-500 to-blue-600 text-white"><div className="flex items-center justify-between"><div><p className="text-blue-100">Total Students</p><p className="text-3xl font-bold mt-1">{stats.totalStudents}</p></div><Users className="w-10 h-10 text-blue-200" /></div></div>
          ...
          <div className="card bg-gradient-to-r from-yellow-500 to-yellow-600 text-white"><div className="flex items-center justify-between"><div><p className="text-yellow-100">Pending</p><p className="text-3xl font-bold mt-1">{stats.totalStudents - stats.verifiedStudents}</p></div><Clock className="w-10 h-10 text-yellow-200" /></div></div>
          <div className="card bg-gradient-to-r from-purple-500 to-purple-600 text-white"><div className="flex items-center justify-between"><div><p className="text-purple-100">Active Jobs</p><p className="text-3xl font-bold mt-1">{stats.activeJobs}</p></div><Briefcase className="w-10 h-10 text-purple-200" /></div></div>
        </div>
