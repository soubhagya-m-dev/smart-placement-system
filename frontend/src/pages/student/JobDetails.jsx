      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="flex items-center gap-6 mt-4 text-gray-500">
                ...
                <span className="flex items-center gap-2"><DollarSign className="w-5 h-5" />{(job.salary?.min / 100000).toFixed(1)} - {(job.salary?.max / 100000).toFixed(1)} LPA</span>
                <span className="flex items-center gap-2"><Briefcase className="w-5 h-5" />{job.jobType}</span>
                <span className="flex items-center gap-2"><Calendar className="w-5 h-5" />Deadline: {new Date(job.applicationDeadline).toLocaleDateString()}</span>
              </div>
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="card">
            ...
            <div className="space-y-2">{job.qualification?.map((q, i) => <p key={i} className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" />{q}</p>) || <p className="text-gray-500">Not specified</p>}</div>
          </div>
        </div>
      </main>
    </div>
